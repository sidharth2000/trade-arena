...
import {
  Box, Container, Grid, Card, CardActionArea, CardMedia, CardContent,
  Typography, Chip, Button, Pagination, CircularProgress,
  TextField, MenuItem, Select, FormControl, InputLabel,
  InputAdornment, Skeleton, Alert, Badge
} from '@mui/material'
import { useTheme } from '@mui/material/styles'
import { Search, Gavel, Package, Timer, TrendingUp, Filter } from 'lucide-react'
  ...
  return (
    ...
    >
      <CardActionArea>
        ...
        <Box
          sx={{
            height: 180, background: isAuction
              ? `linear-gradient(135deg, ${navBg}22, ${navBg}44)`
              : '#f5f5f5',
            display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative'
          }}
        >
          <Package size={48} color={isAuction ? navBg : '#bbb'} />
          {isAuction && (
            <Chip
              label="LIVE AUCTION"
              size="small"
...
export default function ProductsPage() {
  ...
  return (
    <Box sx={{ background: theme.palette.background.default, minHeight: '100vh', pb: 6 }}>
      ...
      {/* Search bar strip */}
      <Box sx={{ background: navBg, py: 1.5, px: 2 }}>
      ...
      <Container maxWidth="lg" sx={{ mt: 3 }}>
        ...
        {(auctionLoading || auctionProducts.length > 0) && (
          <Box sx={{ mb: 4 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 2 }}>
              ...
              <Badge
                badgeContent={auctionProducts.length}
                color="error"
                sx={{ '& .MuiBadge-badge': { fontSize: 11, fontWeight: 700 } }}
              />
              <Box sx={{ flex: 1 }} />
              <Button
                size="small" variant="outlined" startIcon={<TrendingUp size={14} />}
        ...
        {auctionProducts.length > 0 && (
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 3 }}>
            ...
            <Typography variant="caption" color="text.secondary" fontWeight={600}>ALL PRODUCTS</Typography>
            <Box sx={{ flex: 1, height: 1, background: theme.palette.divider }} />
          </Box>
        )}
        ...
        {loading ? (
          ...
          <Box sx={{ textAlign: 'center', py: 8 }}>
            ...
            <Button
              variant="contained" sx={{ mt: 2, background: navBg }}
